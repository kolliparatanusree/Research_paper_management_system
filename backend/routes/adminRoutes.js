...
router.get('/approved-pids', async (req, res) => {
  try {
    ...
    const formatted = approved.map(doc => ({
      ...
      pid: doc.pid,
      uploadedAt: doc.uploadedAt,
      acceptanceLetter: doc.acceptanceLetter
        ? {
            filename: doc.acceptanceLetter?.filename || null,
            contentType: doc.acceptanceLetter?.contentType || null,
            base64: doc.acceptanceLetter?.data
              ? doc.acceptanceLetter.data.toString('base64')
              : null
          }
        : null,
      indexingProof: doc.indexingProof
        ? {
            filename: doc.indexingProof?.filename || null,
            contentType: doc.indexingProof?.contentType || null,
            base64: doc.indexingProof?.data
              ? doc.indexingProof.data.toString('base64')
              : null
          }
        : null
    }));
    ...
    console.error('Error fetching approved PIDs:', err);
...
//   // ✅ Fetch faculty email
//   const faculty = await User.findOne({ userId: request.userId });
...
//     // ✅ Fetch faculty email from Faculty collection
//     const faculty = await User.findOne({ facultyId: updated.userId });
//     if (!faculty || !faculty.email) {
//       console.error("Faculty email not found for:", updated.facultyId);
//       return res.status(400).json({ message: 'Faculty email not found' });
//     }
...
//     // ✅ Fetch faculty email
//     const faculty = await User.findOne({ facultyId: submission.userId });
//     if (faculty && faculty.email) {
//       const mailOptions = {
//         from: 'tanusreekollipara@gmail.com',
//         to: faculty.email,
